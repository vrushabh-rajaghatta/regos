...
**Status:** 🟢 Complete (8 stories shipped — the original 7 + STORY-008 multi-region blueprints; ready to merge to `main`) · **Branch:** `epic/EPIC-001-regulatory-data-dictionary` · **Process:** [FEATURE-DEVELOPMENT-FLOW.md](../FEATURE-DEVELOPMENT-FLOW.md)
...
| **STORY-008** | Seed CA / AU / IN clinical-trial blueprints (Health Canada CTA, TGA CTN, CDSCO CT-04) | ✅ Done — **seed-only, zero schema** (the metadata thesis: a new authority's dossier is data, not code). Extracted the ICH-harmonized M2–M5 into a shared `AddHarmonizedCtdModules(template, rulePrefix)` helper; only regional Module 1 differs. 3 new submission types + 3 regional M1 form doc types; FDA output preserved exactly (38/13/4). New: HC 36/11/4, TGA(CTN) 36/12/4, CDSCO 36/12/4. Browser specs: `blueprint-seed-integrity` now pins all four; `templates` asserts the 4-template list. Live-verified all four render; 511 backend green |
...
- **The thesis held under a real test (STORY-008).** Adding three more authorities' clinical-trial blueprints (Canada CTA, Australia CTN, India CT-04) was **pure seed data — zero schema, zero domain, no migration**. The ICH-harmonized Modules 2–5 factored into one shared helper; only regional Module 1 differs. This is exactly "regulatory knowledge as versioned data, not code," demonstrated rather than asserted — and the same read API + Explorer render all four unchanged.
