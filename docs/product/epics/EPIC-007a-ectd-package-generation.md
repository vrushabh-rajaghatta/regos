...
| contact roles | **translate at the boundary, map only what evidence supports.** `REG → fdaact1`, ~~`MFG → fdaact2`~~; everything else refused. No token column on `ContactRole` — the taxonomies answer different questions. **`HA-REVIEWER` being unmappable is the point**: an authority reviewer must never appear as an applicant contact. ⚠ **`MFG → fdaact2` was withdrawn on 2026-08-03** — FDA's own list says `fdaact2` is the *Technical Contact* (**E31**), and the M1 example's phrase *"the technical contact"* had been read as a description when it was a name. **The mapping is one row now** |
