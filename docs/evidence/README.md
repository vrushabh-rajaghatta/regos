...
| **E33** | **The STF vocabulary is `valid-values.xml`, not the DTD — and it is 97 values, not ~40.** `file-tag` holds **68 `ich` + 25 `us` + 4 `jp`**; `category` holds `species` (9, ich), `route-of-admin` (8, ich), `duration` (**3, `us`**) and `type-of-control` (5, ich); `property` holds one value, `site-identifier` (us). A category's realm is **per category**, not uniformly `ich` | [`valid-values.xml`](EPIC-019/spec/valid-values.xml) v6.0, counted by parse | **2a** — a normative machine-readable artifact | **EPIC-019 S002b and S003.** **Supersedes E29's "~40 values"**, which was a summary's count and wrong by more than half. FDA's *"22 controlled file tags"* (E21) is the `us` realm as it stood at v4.0 — the 25 today are those 22 plus the three `HF-validation-*` added in v5.0 | EPIC-019 |
| **E34** | **The DTD cannot police the vocabulary, and the stylesheet can.** `file-tag/@name` is declared `CDATA #REQUIRED`, so `xmllint --valid` **accepts `name="sinopsis"`**. The ICH stylesheet resolves every `file-tag`, `category` and `property` against `valid-values.xml` and paints unknown ones `bgcolor="#FF6666"` — 1 red row for `sinopsis`, 0 for `synopsis`, and 1 for `duration` carrying `info-type="ich"` instead of `us` | `xmllint` + `xsltproc` over [`ich-stf-v2-2.dtd`](EPIC-019/spec/ich-stf-v2-2.dtd) and [`ich-stf-stylesheet-2-3.xsl`](EPIC-019/spec/ich-stf-stylesheet-2-3.xsl) | **2a** | **A second oracle, for a class of error the first is blind to.** EPIC-007a's Level 2a rested on one parser answering *"is this legal?"*; this answers *"is this a word?"* — and it is third-party, machine-checkable, and shipped by ICH. The DTD's own comment says so: the valid values are *"controlled by the ICH default stylesheet"*, not by the DTD | EPIC-019 |
| **E35** | **The STF's element order is fixed and enforced.** `study-identifier` is `(title, study-id, category*)` — **title first**; `study-id` before `title` is rejected. `doc-content` is `(title?, property*, file-tag*)` with an optional title we had not modelled; `file-tag` may itself contain `property*`; `content-block` is a hierarchical alternative to `doc-content`. `ectd:study` takes exactly one of each child, and `dtd-version` is `#FIXED "2.2"` | `xmllint --valid` negative controls over [`ich-stf-v2-2.dtd`](EPIC-019/spec/ich-stf-v2-2.dtd) | **2a** | **EPIC-019 S003.** The stylesheet reads `valid-values.xml` by a **relative** path, so both files ship together in `util/style/` — which is what ADR-054 recorded as an absent folder without knowing its contents | EPIC-019 |
...
| **E36** | **RegOS does not hold ISO 3166-1 — it holds eight hand-verified rows.** The alpha-3 code and the official ISO name are **not derivable** from the alpha-2 code or the common name, in either direction; six of the eight seeded countries have an ISO name identical to the common one, which is exactly the trap a rule inferred from the majority would fall into | ISO 3166-1, read off the freely published Online Browsing Platform — [the eight, and what is not held](EPIC-022/iso-3166-1.md) | **3** | **EPIC-022 S001.** The register itself is licensed and absent, so this entry stops covering the seed the moment a ninth country is added. **Blast radius is EPIC-007b**: a wrong alpha-3 produces a well-formed message naming the wrong country. ⚠ The register itself remains licensed and absent | EPIC-022 |
| **E37** | **Australia and India are ICH _observers_, not members — and India is not a PIC/S participant, so it belongs to none of the five groupings RegOS records.** Membership has **no single authority**: each grouping publishes only its own list, so this is three sources read separately. The United Kingdom is **not** EU, which is the row showing membership is a dated fact rather than geography | [EU — 27 member states](https://european-union.europa.eu/principles-countries-history/eu-countries_en) · [ICH — Members & Observers](https://www.ich.org/page/members-observers) · [PIC/S — Members](https://picscheme.org/en/members), with accession dates · [the eight, and the two corrections](EPIC-022/regional-membership.md) | **3** | **EPIC-022 S002.** Both observer rows would have been seeded as members from memory, and an observer does not adopt ICH guidelines by obligation — so the field's own question gets the opposite answer for both. **Germany and France are tagged ICH by inheritance**, recorded as a derived claim rather than a register row. ⚠ **No effective dating**: RegOS records today's membership and nothing detects a change | EPIC-022 |
| **E38** | **"Canada requires English and French" is not true as a rule** — the bilingual obligation covers the Product Monograph and most labels (`C.01.014.1(2)(m.1)`, `C.08.002(2)(j.1)`, in force 2015-06-13) but **not** prescription-only, hospital-only or professional-use labels, which may be in one language. Which rule applies depends on the product and the document, and **a country knows neither** | [Health Canada — labelling of pharmaceutical drugs for human use](https://www.canada.ca/en/health-canada/services/drugs-health-products/drug-products/applications-submissions/guidance-documents/labelling-pharmaceutical-drugs-humans.html) · [the eight, and which were sourced](EPIC-022/label-languages.md) | **3** | **EPIC-022 S003, and it changed the design.** The natural implementation was a validation refusing an incomplete Canadian label set; it would have been wrong for every hospital-only product. The field is therefore *expected*, not *required*, and the check is advisory (D4). ⚠ **Only the Canada row was read from a labelling source** — the other seven are the market's official language, a reasonable stand-in and a different claim | EPIC-022 |
| **E39** | **The authoritative source publishes storage conditions, not climatic zones — and India accepts 30 °C/70% RH, which is neither Zone IVA (30/65) nor Zone IVB (30/75).** WHO lists the long-term testing condition each member state accepts and publishes no zone letter per country; **ICH withdrew Q1F**, which is where zone letters came from. Seven of the eight seeded markets accept *25 °C/60% RH **or** 30 °C/65% RH* — WHO's own "or", and the reason the match is an overlap | [WHO — *Stability conditions for WHO Member States by Region*, update March 2021](https://cdn.who.int/media/docs/default-source/medicines/norms-and-standards/guidelines/regulatory-standards/trs953-annex2-appendix1-stability-conditions-table-2018.pdf) (previously Table 2 in Annex 2 to TRS 953) · [the eight, and the footnote that grades them](EPIC-022/stability-conditions.md) | **3** | **EPIC-022 S004, and it killed the planned abstraction.** The design was a `ClimaticZone` column matched on zone letters; storing `IVB` for India would have been RegOS's *interpretation* of WHO's data with nothing to check it against. **D6 was amended in place before a line was written.** ⚠ **The table grades its own rows and they are not equal**: **Australia is footnote 2** — collated at the 13th ICDRA in 2008 — where the other seven are official communications from national regulators to WHO | EPIC-022 |
...
| [**ADR-056**](../adr/ADR-056-study-identity-is-owned-by-the-sponsor.md) — study identity is owned by the sponsor | **E21**, **E24**, **E29** | **E24 is load-bearing for the ownership, not merely for the modelling.** E29 says the `study-id` is *the sponsor's*; E24 says it must be byte-identical across sequences or FDA's tooling splits one study into two — and **an entity owned by a submission cannot promise that**. Ownership by the sponsor is what turns the stability into an invariant rather than a hope. E21 is why it is urgent: every IND has Module 4 content |
...
### Resolved, 2026-08-03 — the vocabulary arrived, and it was in a different file

**`valid-values.xml` v6.0, `ich-stf-v2-2.dtd` and `ich-stf-stylesheet-2-3.xsl`
are now held** ([spec/](EPIC-019/spec/)). The correction below stands as
written, and **one sentence in it was itself wrong**: it said the enumeration
*"lives in `ich-stf-v2-2.dtd`"*. It does not. `file-tag/@name` is `CDATA`, and
`xmllint` validates a misspelled tag without complaint (**E34**). The vocabulary
lives in `valid-values.xml`, which the DTD's own comment points at —
*"controlled by the ICH default stylesheet"*.

> **Two files, two jobs, and guessing which was which cost nothing only because
> nothing was built on the guess.** The DTD answers *is this legal?*; the
> stylesheet plus `valid-values.xml` answers *is this a word?* The backlog named
> one file as the source of both.

Recorded as **E33**, **E34** and **E35**. E29's *"~40 values"* is superseded: the
real count is **97**.
