...
internal static class RegulatoryTemplates
{
    ...
    private static RegulatoryTemplate BuildFdaIndCtd()
    {
        ...
        // ── v3 — FDA's own wording, and where each section is written ────────
        //
        // One version for two changes, because both are blueprint knowledge and
        // splitting them would leave two immutable versions where one semantic
        // change happened (ADR-052).
        //
        //   · titles corrected to FDA's own — "Investigational Brochure", not
        //     "Investigator's Brochure"; "Cover Letters", not "Cover Letter".
        //     S002 moved the brochure to 1.14.4.1 and carried its old caption
        //     across; FDA's DTD element and its Comprehensive ToC agree with
        //     each other and not with us.
        //   · every section gains its eCTD folder, with the provenance of that
        //     folder recorded beside it.
        var v3 = template.StartDraftVersion();
        AddFdaIndModule1WithPlacement(template);
    ...
    /// <summary>
    /// FDA Module 1 with FDA's own wording and every section's eCTD folder.
    /// </summary>
    /// <remarks>
    /// <b>Module 1 is where Appendix 4 stops.</b> It gives <c>m1</c> and one
    /// regional directory per region — <c>m1/us</c> here — and then defers to
    /// regional guidance, which supplies a heading hierarchy and no directory
    /// column at all. So the module root is the specification's and everything
    /// under it is <b>RegOS's own</b>, generated by ADR-052's convention:
    /// the section identifier without dots, then a concise kebab-case slug.
    /// <para>
    /// The two are stored with different <see cref="EctdFolderSource"/> values
    /// precisely so that nobody later reads <c>11441-invest-brochure</c> as
    /// something FDA published. It is not weaker evidence; it is not evidence.
    /// </para>
    /// </remarks>
    private static void AddFdaIndModule1WithPlacement(RegulatoryTemplate template)
    {
        const EctdFolderSource Ich = EctdFolderSource.IchAppendix4;
        const EctdFolderSource Ours = EctdFolderSource.RegOsConvention;

        // Appendix 4 #3 and #6. Two directories, one section — the regional
        // level has no section of its own to carry it.
        ...
        // FDA's element is m1-14-4-1-investigational-brochure and its
        // Comprehensive ToC v2.3.2 says "Investigational brochure". Two FDA
        // sources agreeing with each other and not with RegOS is not a wording
        // preference.
    ...
    /// </summary>
    /// <param name="folderSource">
    /// Null for versions published before eCTD placement was modelled — and
    /// they must stay that way. <b>The seed reproduces history</b>, so adding
    /// folders to an already-published version here would give a fresh clone a
    /// different past from an upgraded database (the per-version initializer
    /// never revisits a version it has already written).
    /// </param>
    private static void AddHarmonizedCtdModules(
        RegulatoryTemplate template, string rulePrefix,
        EctdFolderSource? folderSource = null)
    {
        // Every folder below is ICH Appendix 4's own value, relative to its
        // parent section. Where our tree omits an intermediate node that the
        // appendix has — RegOS goes M3 -> 3.2.S, the appendix goes 3 -> 3.2 ->
