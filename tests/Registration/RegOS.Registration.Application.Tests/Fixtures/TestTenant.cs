...
namespace RegOS.Registration.Application.Tests.Fixtures;

/// <summary>
/// The single tenant every Registration integration test works under. One fixed
/// tenant rather than a fresh one per fixture: the global query filter
/// (ADR-031) only shows rows to the tenant that owns them, so every context and
/// every created aggregate must agree on it — and the holder organization the
/// tests use has to be one this tenant can see.
/// </summary>
